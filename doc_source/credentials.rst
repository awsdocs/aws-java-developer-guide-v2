...
by the :aws-java-class-prev:`DefaultCredentialsProvider <auth/credentials/DefaultCredentialsProvider>`
...
#. **Java system properties** |ndash| :code:`aws.accessKeyId` and :code:`aws.secretAccessKey`.
   The |sdk-java| uses the :aws-java-class-prev:`SystemPropertyCredentialsProvider <auth/credentials/SystemPropertyCredentialsProvider>`
   to load these credentials.

#. **Environment variables** |ndash| :envvar:`AWS_ACCESS_KEY_ID` and :envvar:`AWS_SECRET_ACCESS_KEY`.
   The |sdk-java| uses the :aws-java-class-prev:`EnvironmentVariableCredentialsProvider <auth/credentials/EnvironmentVariableCredentialsProvider>`
...
#. **The default credential profiles file** |ndash| typically located at :file:`~/.aws/credentials`
   ...
   |sdk-java| uses the :aws-java-class-prev:`ProfileCredentialsProvider <auth/credentials/ProfileCredentialsProvider>` to load these credentials.
...
#. **Amazon ECS container credentials** |ndash| loaded from the |ECS| if the environment
   ...
   :aws-java-class-prev:`ContainerCredentialsProvider <auth/credentials/ContainerCredentialsProvider>` to load these
   credentials.
#. **Instance profile credentials** |ndash| used on |EC2| instances, and delivered through the |EC2|
   metadata service. The |sdk-java| uses the :aws-java-class-prev:`InstanceProfileCredentialsProvider
...
takes an :aws-java-class-prev:`AwsCredentialsProvider <auth/credentials/AwsCredentialsProvider>` interface as input. The
...
see **All Known Implementing Classes** in :aws-java-class-prev:`AwsCredentialsProvider <auth/credentials/AwsCredentialsProvider>`.
...
.. topic:: To explicitly supply credentials to an AWS client
    ...
    #. Instantiate a class that provides the :aws-java-class-prev:`AwsCredentials <auth/credentials/AwsCredentials>`
       interface, such as :aws-java-class-prev:`AwsSessionCredentials <auth/credentials/AwsSessionCredentials>`. Supply
       it with the AWS access key and secret key you will use for the connection.

    #. Create an :aws-java-class-prev:`StaticCredentialsProvider <auth/credentials/StaticCredentialsProvider>` with
