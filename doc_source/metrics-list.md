...
|  ConcurrencyAcquireDuration  |  The time taken to acquire a channel from the connection pool  |  Duration  |  Yes  | 
...
|  LocalStreamWindowSize  |  The local HTTP/2 window size in bytes for the stream that this request was executed on  |  Integer  |  Yes  | 
...
|  RemoteStreamWindowSize  |  The remote HTTP/2 window size in bytes for the stream that this request was executed on  |  Integer  |  Yes  | 
